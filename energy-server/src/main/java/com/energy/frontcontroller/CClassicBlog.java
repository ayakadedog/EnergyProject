...
public class CClassicBlog {
    ...
    public Result<Page> frontpage(Integer page, Integer pageSize, String category, String title) {
        //构造分页构造器对象
        Page<CSupport> pageInfo = new Page<>(page, pageSize);
        ...
        LambdaQueryWrapper<CSupport> queryWrapper = new LambdaQueryWrapper<>();
        //添加过滤条件
        queryWrapper.like(category != null, CSupport::getCategory, category);
        queryWrapper.like(title != null, CSupport::getTitle, title);
        //添加排序条件
        queryWrapper.orderByDesc(CSupport::getId);
